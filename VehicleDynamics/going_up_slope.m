...
% セダン型の車両の描画データ
vehicle_shape_x = [-2, -0.5, 1.5, 2, 3.5, 3.5, 2, 1.5, -1.5, -2, -2];
...
plot([positions(1,1), positions(end,1)], [0, 0], 'k-', 'LineWidth', 2); % 地面のラインを描画

% 壁の描画
brick_wall_x = [positions(end,1), positions(end,1), positions(end,1) + 1, positions(end,1) + 1];
brick_wall_z = [0, positions(end,2) + 10, positions(end,2) + 10, 0];
fill(brick_wall_x, brick_wall_z, [0.8, 0.3, 0]); % 茶色の壁を描画
