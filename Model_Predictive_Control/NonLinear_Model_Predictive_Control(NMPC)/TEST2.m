...
function dF = dFdu( obj, x_current, u, system, nmpc )
    ...
    for cnt = 1:nmpc.N*nmpc.len_u
        ...
        dF(:,cnt) = ( F( obj, x_current, u_buff_p, system, nmpc ) - F( obj, x_current, u_buff_n, system, nmpc ) ) / ( 2 * diff );
    end
end
...
function ans_F = F( obj, x_current, u, system, nmpc )
    ...
    for cnt = 1:nmpc.N
        ans_F((1:nmpc.len_u)+nmpc.len_u*(cnt-1)) = obj.Hu(x((1:nmpc.len_x)+nmpc.len_x*(cnt-1)), ...
                                                          u((1:nmpc.len_u)+nmpc.len_u*(cnt-1)), ...
                                                          lmd((1:nmpc.len_lmd)+nmpc.len_lmd*(cnt-1)),...
                                                          nmpc.r );
    end
end
...
function lmd = Backward( obj, x, u, nmpc )
    ...
    lmd((1:nmpc.len_lmd)+nmpc.len_lmd*(nmpc.N-1)) = obj.phix( x((1:nmpc.len_x)+nmpc.len_x*(nmpc.N-1)), nmpc.sf );
    
    for cnt = nmpc.N-1:-1:1                                                    
        lmd((1:nmpc.len_lmd)+nmpc.len_lmd*(cnt-1)) = lmd((1:nmpc.len_lmd)+nmpc.len_lmd*(cnt)) ...
                                                   + obj.Hx( x((1:nmpc.len_x)+nmpc.len_x*(cnt)), u, lmd((1:nmpc.len_lmd)+nmpc.len_lmd*(cnt)), nmpc.q ) * dt;
    end
end
