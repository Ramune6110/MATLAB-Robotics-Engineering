...
function [xv, lmdv, uv, muv, fxu, Cxu] = defineSystem(system, nmpc)
    ...
    fxu = [xv(4);
           ...
           ( uv(1) * system.r + uv(2) * system.r ) / system.I];            % state equation
    Cxu = [(uv(1) - nmpc.uavg)^2 + uv(3)^2 - (nmpc.umax - nmpc.uavg)^2;
           (uv(2) - nmpc.uavg)^2 + uv(4)^2 - (nmpc.umax - nmpc.uavg)^2]; % constraints
end
...
function [L, phi, qv, rv, kv, sfv] = evaluationFunction(xv, uv, nmpc)
    ...
    L   = simplify( ((xv - nmpc.xf)' * Q * (xv - nmpc.xf)) / 2 + (uv' * R * uv) / 2 - kv(1) * uv(3) - kv(2) * uv(4) ); % Integrand in the performance matrix
    phi = simplify( ((xv - nmpc.xf)' * Sf * (xv - nmpc.xf)) / 2 );                                      % terminal penalty
end
