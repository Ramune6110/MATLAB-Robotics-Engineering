...
ny = 1;   % Number of ovservation
nu = 1;   % Number of input

S = 10.0; % Termination cost weight matrix
Q = 1.0; % Weight matrix of state quantities
R = 1.0;              % Weight matrix of input quantities
...
% states = [y, psi];
% https://www.youtube.com/watch?v=47LzXHOXwtU
system.A = eye(nx) + [0.0, Vx; 0.0, 0.0] * dt;
system.B = [0.0; 1.0] * dt;
system.C = [1.0, 0.0];
...
for i = 1:ny:length(refSignals)
    refSignals(i) = y_ref(k, 1);
end

% initial state
x0 = [y_ref(1, 1); psi_ref(1, 1)]; % Initial state of Lateral Vehicle Dynamics
...
function drow_figure(xTrue, uk, x_ref, y_ref, current_step, trajectory_type)
    ...
    if strcmp(trajectory_type, 'Lane change')
        ...
        hold on
        plot(x_ref(1: current_step, 1),xTrue(1, 1:current_step),'r','LineWidth',1)
        hold on;
        grid on;
        axis equal;
